...
class Preference():
    ...
    def __init__(self, rankMap, wmgMap, count=1):
        self.rankMap = rankMap  # ----- Updated on May 1, 2019 ----------
        self.wmgMap = wmgMap
        self.count = count
    ...
    def getOrderVector(self):  # ----- Updated on May 1, 2019 ----------
        """
        ...
        # We sort the candidates based on the number of incoming edges they have in the graph. If
        # two candidates have the same number, we assume that they are tied.
        # incEdgesMap = self.rankMap.keys
        if len(self.rankMap.keys()) == 1:  # ----- Updated on May 1, 2019 ----------
            incEdgesMap = {0: list(self.rankMap.keys())}  # ----- Updated on May 1, 2019 ----------
            sortedKeys = [0]  # ----- Updated on May 1, 2019 ----------
        else:
            incEdgesMap = self.getIncEdgesMap()
            sortedKeys = sorted(incEdgesMap.keys(), reverse=True)
