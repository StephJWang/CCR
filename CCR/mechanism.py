...
class MechanismPosScoring(Mechanism):
    ...
    def getCandScoresMap(self, profile, normalize=False):
        """
        ...
        elecType = profile.getElecType()
        if elecType != "soc" and elecType != "toc" and elecType != "soi":
        ...
        for i in range(0, len(rankMaps)):
            ...
            # print("ok", rankMap, rankMapCount)
            for cand in rankMap.keys():
                candScoresMap[cand] += scoringVector[rankMap[cand] - 1] * rankMapCount

        if normalize is True:
            n = profile.numVoters
            max_score = max(scoringVector)
            for cand in candScoresMap.keys():
                candScoresMap[cand] /= max_score * n
