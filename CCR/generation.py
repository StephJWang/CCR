...
class strict_order(generation):
    ...
    def generation_patch(self, start=0):
        ...
        if self.IsComplete is False:
            extension = 'soi'
        folder = 'M'+str(m)+'N'+str(n)+'-'+extension+'-Mode'+str(self.Mode)
        os.mkdir(folder)
        os.chdir(folder)
        for i in range(self.num):
            file_name = 'M'+str(m)+'N'+str(n)+'-'+str(start+i+1).zfill(len(str(self.num)))+'.'+extension
            f = open(file_name, 'w+')
            txt = self.generation_one()
