...
def CCR_ILP(profile, k=2):
    """
    Returns the winner committee of given profile under Chamberlain-Courant rule
    ...
    model.addConstrs(gp.quicksum(y[i, j] for i in range(m)) == 1 for j in range(b))

    alpha = gp.quicksum(prefcounts[j] * y[i, j] * (m - RankMaps[j][i+1]) for j in range(b) for i in range(m))
